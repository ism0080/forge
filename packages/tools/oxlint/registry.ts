...
const defineRuleRegistry = <const Registry extends Record<string, RuleDefinition>>(
  registry: Registry,
): Readonly<Registry> => Object.freeze(registry);

export const ruleDefinitions = defineRuleRegistry({
  "no-array-filter-map": { rule: noArrayFilterMapRule, profile: "core", severity: "error" },
  "no-chained-type-assertions": { rule: noChainedTypeAssertionsRule, profile: "core", severity: "error" },
  "no-conditional-empty-object-spread": { rule: noConditionalEmptyObjectSpreadRule, profile: "core", severity: "error" },
  "no-known-value-widening": { rule: noKnownValueWideningRule, profile: "core", severity: "error" },
  "no-module-mocking": { rule: noModuleMockingRule, profile: "core", severity: "error" },
  "no-object-parameters": { rule: noObjectParametersRule, profile: "core", severity: "error" },
  "no-reduce-accumulator-copy": { rule: noReduceAccumulatorCopyRule, profile: "core", severity: "error" },
  "no-reflect-apply": { rule: noReflectApplyRule, profile: "core", severity: "error" },
  "no-reflect-get": { rule: noReflectGetRule, profile: "core", severity: "error" },
  "no-runtime-typeof": { rule: noRuntimeTypeofRule, profile: "core", severity: "error" },
  "no-shape-in-symbol-names": { rule: noShapeInSymbolNamesRule, profile: "core", severity: "error" },
  "no-unknown-parameters": { rule: noUnknownParametersRule, profile: "core", severity: "error" },
  "no-unknown-returns": { rule: noUnknownReturnsRule, profile: "core", severity: "error" },
  "no-unknown-type-aliases": { rule: noUnknownTypeAliasesRule, profile: "core", severity: "error" },
  "no-unsafe-dictionary-type": { rule: noUnsafeDictionaryTypeRule, profile: "core", severity: "error" },
  "no-widen-then-assert": { rule: noWidenThenAssertRule, profile: "core", severity: "error" },
  "require-safety-comment-for-type-assertion": { rule: requireSafetyCommentForTypeAssertionRule, profile: "core", severity: "error" },
  "no-ambient-nondeterminism": { rule: noAmbientNondeterminismRule, profile: "effect", severity: "error" },
  "no-cascading-layer-provide": { rule: noCascadingLayerProvideRule, profile: "effect", severity: "error" },
  "no-direct-browser-storage": { rule: noDirectBrowserStorageRule, profile: "effect", severity: "error" },
  "no-direct-fetch": { rule: noDirectFetchRule, profile: "effect", severity: "error" },
  "no-disable-validation": { rule: noDisableValidationRule, profile: "effect", severity: "error" },
  "no-effect-as-void": { rule: noEffectAsVoidRule, profile: "effect", severity: "error" },
  "no-global-json": { rule: noGlobalJsonRule, profile: "effect", severity: "error" },
  "no-in-operator": { rule: noInOperatorRule, profile: "effect", severity: "error" },
  "no-nested-effect-array-methods": { rule: noNestedEffectArrayMethodsRule, profile: "effect", severity: "error" },
  "no-nested-layer-provide": { rule: noNestedLayerProvideRule, profile: "effect", severity: "error" },
  "no-service-constructor-imports": { rule: noServiceConstructorImportsRule, profile: "effect", severity: "error" },
  "no-service-option": { rule: noServiceOptionRule, profile: "effect", severity: "error" },
  "no-shadowed-standard-array-static": { rule: noShadowedStandardArrayStaticRule, profile: "effect", severity: "error" },
  "no-silent-error-swallow": { rule: noSilentErrorSwallowRule, profile: "effect", severity: "error" },
  "no-static-effect-service-forwarders": { rule: noStaticEffectServiceForwardersRule, profile: "effect", severity: "error" },
  "no-switch": { rule: noSwitchRule, profile: "effect", severity: "error" },
  "no-try-catch": { rule: noTryCatchRule, profile: "effect", severity: "error" },
  "pipe-max-arguments": { rule: pipeMaxArgumentsRule, profile: "effect", severity: "error" },
  "prefer-effect-match": { rule: preferEffectMatchRule, profile: "effect", severity: "error" },
  "prefer-option-from-nullable": { rule: preferOptionFromNullableRule, profile: "effect", severity: "error" },
  "no-fixed-height-on-content": { rule: noFixedHeightOnContentRule, profile: "react", severity: "error" },
  "no-direct-xstate-create-machine": { rule: noDirectXstateCreateMachineRule, profile: "xstate", severity: "error" },
  "no-direct-xstate-use-selector": { rule: noDirectXstateUseSelectorRule, profile: "xstate", severity: "error" },
  "no-multiple-xstate-hooks": { rule: noMultipleXstateHooksRule, profile: "xstate", severity: "error" },
  "no-react-state-hooks": { rule: noReactStateHooksRule, profile: "xstate", severity: "error" },
  "no-single-use-xstate-actions": { rule: noSingleUseXstateActionsRule, profile: "xstate", severity: "error" },
  "no-single-use-xstate-guards": { rule: noSingleUseXstateGuardsRule, profile: "xstate", severity: "error" },
  "require-xstate-event-satisfies": { rule: requireXstateEventSatisfiesRule, profile: "xstate", severity: "error" },
});
